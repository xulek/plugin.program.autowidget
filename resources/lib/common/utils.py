...
colors = ['lightsalmon', 'salmon', 'darksalmon', 'lightcoral', 'indianred', 'crimson', 'firebrick', 'red', 'darkred',
          # red
          ...
          'mediumseagreen', 'seagreen', 'olive', 'darkolivegreen', 'olivedrab',  # green
          'lightcyan', 'cyan', 'aqua', 'aquamarine', 'mediumaquamarine', 'paleturquoise', 'turquoise',
          'mediumturquoise', 'darkturquoise', 'lightseagreen', 'cadetblue', 'darkcyan', 'teal',  # cyan
          ...
          'mediumslateblue', 'slateblue', 'darkslateblue',  # blue
          'lavender', 'thistle', 'plum', 'violet', 'orchid', 'fuschia', 'magenta', 'mediumorchid', 'mediumpurple',
          'blueviolet', 'darkviolet', 'darkorchid', 'darkmagenta', 'purple', 'indigo',  # purple
          ...
          'beige', 'oldlace', 'floralwhite', 'ivory', 'antiquewhite', 'linen', 'lavenderblush', 'mistyrose',  # white
          'gainsboro', 'lightgray', 'silver', 'darkgray', 'gray', 'dimgray', 'lightslategray', 'slategray',
          'darkslategray', 'black',  # black
          'cornsilk', 'blanchedalmond', 'bisque', 'navajowhite', 'wheat', 'burlywood', 'tan', 'rosybrown', 'sandybrown',
          'goldenrod', 'peru', 'chocolate', 'saddlebrown', 'sienna', 'brown', 'maroon']  # brown
...
def cache_files(path, widget_id):
    hash = hashlib.sha1(six.text_type(path).encode("utf-8")).hexdigest()
...
def cache_expiry(hash, widget_id, add=None, no_queue=False):
    ...
    log("{} cache {}B (exp:{}, last:{}): {} {}".format(result, size, ft(expiry - time.time()), ft(since_read), hash[:5],
                                                       widgets), 'notice')
...
def predict_update_frequency(history):
    ...
    log("avg_dur {:0.0f}s, med_dur {:0.0f}s, weighted {:0.0f}s, ones {:0.2f}, all {}".format(avg_dur, med_dur, weighted,
                                                                                             ones, changes), "debug")
    if ones > 0.9:
...
def widgets_changed_by_watching(media_type):
    ...
    priority = sorted([(chance_playback_updates_widget(path, plays_for_type), path) for path in all_cache],
                      reverse=True)
